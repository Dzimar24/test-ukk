...
(function () {
	...
	const viewport = (origin, optBounds) =>
		optBounds.fold(
	...
	const ancestors = (scope, predicate, isRoot) =>
		...
		const selectionBox = getAnchorSelection(win, anchorInfo).bind((sel) => {
			if (isSimRange(sel)) {
				const optRect = getBounds$1(
					...
					const rect = getFirstRect(
						win,
						SimSelection.exact(zeroWidth$1, 0, zeroWidth$1, 1),
					);
				...
				const selectionRect = map$1(sel, (cell) =>
					cell.dom.getBoundingClientRect(),
				);
				const bounds = {
					left: Math.min(
						selectionRect.firstCell.left,
						selectionRect.lastCell.left,
					),
					right: Math.max(
						selectionRect.firstCell.right,
						selectionRect.lastCell.right,
					),
					top: Math.min(
						selectionRect.firstCell.top,
						selectionRect.lastCell.top,
					),
					bottom: Math.max(
						selectionRect.firstCell.bottom,
						selectionRect.lastCell.bottom,
					),
				};
				return getBox(
					bounds.left,
	...
	const positionWithinBounds = (
		...
		preserve$1(() => {
			...
			anchorage.placement(component, anchorage, origin).each((anchoring) => {
				const optBounds = optWithinBounds.orThunk(() =>
					posConfig.getBounds.map(apply$1),
				);
	...
	const create$2 = (cyclicField) => {
		...
		const isFirstChild = (elem) =>
			parentNode(elem)
				.bind(firstChild)
				.exists((child) => eq(child, elem));
		const goFromPseudoTabstop = (component, simulatedEvent, tabbingConfig) =>
			findCurrent(component, tabbingConfig)
				.filter((elem) => !tabbingConfig.useTabstopAt(elem))
				.bind((elem) =>
					(isFirstChild(elem) ? goBackwards : goForwards)(
						component,
						simulatedEvent,
						tabbingConfig,
					),
	...
	const f = (container, selector, current, delta, getNewIndex) => {
		...
		const tryNewIndex = (initial, index, candidates) =>
			getNewIndex(
				initial,
				index,
				delta,
				...
				candidates[index],
				(newIndex) =>
					isDisabledButton(candidates[newIndex])
	...
	const horizontalWithoutCycles = (container, selector, current, delta) =>
		f(
			container,
			selector,
			current,
			delta,
			(prevIndex, v, d, min, max, oldCandidate, onNewIndex) => {
				const newIndex = clamp(v + d, min, max);
				return newIndex === prevIndex
					? Optional.from(oldCandidate)
					: onNewIndex(newIndex);
			},
		);
	const horizontal = (container, selector, current, delta) =>
		f(
			...
			current,
			delta,
			(prevIndex, v, d, min, max, _oldCandidate, onNewIndex) => {
	...
	const moveLeft$2 = (element, focused, info) =>
		(info.cycles ? horizontal : horizontalWithoutCycles)(
			element,
			info.selector,
			focused,
			-1,
	...
	const getKeydownRules$3 = (_component, _se, flowConfig, _flowState) => {
		const westMovers = [...(flowConfig.allowHorizontal ? LEFT : [])].concat(
			flowConfig.allowVertical ? UP : [],
		);
		const eastMovers = [...(flowConfig.allowHorizontal ? RIGHT : [])].concat(
	...
	) => renderIcon$3(spec, iconName, iconProvider(), fallbackIcon);
	...
	const setup$e = (api, editor) => {
		...
		editor.on("NodeChange", (e) => {
			if (
				...
				detect$1(SugarElement.fromDom(e.element)).isNone()
			) {
				api.cancelIfNecessary();
			}
		});
	...
	const setupReadonlyModeSwitch = (editor, uiRefs) => {
		...
		editor.on("SwitchMode", () =>
			broadcastReadonly(uiRefs, editor.mode.isReadOnly()),
		);
	...
	const renderIcon$2 = (name, icons, classes = [iconClass]) =>
		render$3(
			name,
			{
				tag: "div",
	...
	const renderCheckmark = (icons) =>
		renderIcon$2("checkmark", icons, [checkmarkClass]);
	const renderSubmenuCaret = (icons) =>
		renderIcon$2("chevron-right", icons, [caretClass]);
	const renderDownwardsCaret = (icons) =>
		renderIcon$2("chevron-down", icons, [caretClass]);
	...
	const onSetupEditableToggle = (editor) =>
	...
	const getCacheForId = (id) =>
		get$g(cacheStorage, id).getOrThunk(() => {
			...
			if (isNullable(currentData)) {
				...
				global$4.setItem(
					storageId,
					isNonNullable(legacyDefault) ? legacyDefault : "[]",
				);
			}
	...
	const getColors$2 = (editor, id) => {
		if (id === foregroundId && editor.options.isSet("color_map_foreground")) {
			...
			id === backgroundId &&
			editor.options.isSet("color_map_background")
		) {
	...
	const calcCols = (editor, id = "default") =>
		Math.max(
			fallbackCols,
			Math.ceil(Math.sqrt(getColors$2(editor, id).length)),
		);
	...
	const isValidBackgroundColor = (value) =>
		fromString(value).exists((c) => c.alpha !== 0);
	...
	const getColors$1 = (colors, id, hasCustom) =>
	...
	const registerTextColorButton = (
		...
		editor.ui.registry.addSplitButton(name, {
			...
			fetch: getFetch$1(
				getColors$2(editor, format),
				format,
				hasCustomColors$1(editor),
			),
	...
	const getColorItems = (spec, backstage) => {
		...
		return spec.initData.colors.fold(
			() =>
				getColors$1(
					backstage.colorinput.getColors(spec.initData.storageKey),
					spec.initData.storageKey,
					useCustomColors,
				),
	...
	const makeAnnouncementText = (backstage) => (row, col) =>
	...
	const renderNestedItem = (
		...
		const getApi = (component) => ({
			...
			setIconFill: (id, value) => {
				descendant(
					component.element,
					`svg path[class="${id}"], rect[class="${id}"]`,
				).each((underlinePath) => {
	...
	const getAutocompleterRange = (dom, initRange) => {
		return detect$1(SugarElement.fromDom(initRange.startContainer)).map(
			(elm) => {
	...
	const isScrollingOverflowValue = (value) =>
		trim$1(value).length > 0 && !contains$2(nonScrollingOverflows, value);
	const isScroller = (elem) => {
		if (isHTMLElement(elem)) {
			const overflowX = get$e(elem, "overflow-x");
			const overflowY = get$e(elem, "overflow-y");
			return (
				isScrollingOverflowValue(overflowX) ||
				isScrollingOverflowValue(overflowY)
			);
		} else {
			return false;
		}
	...
	const detect = (popupSinkElem) => {
		...
		const scrollers =
			ancestorsScrollers.length === 0
				? getShadowRoot(popupSinkElem)
						.map(getShadowHost)
						.map((x) => ancestors(x, isScroller))
						.getOr([])
				: ancestorsScrollers;
	...
	const detectWhenSplitUiMode = (editor, popupSinkElem) =>
		isSplitUiMode(editor) ? detect(popupSinkElem) : Optional.none();
	...
	const renderLabel$3 = (label, providersBackstage) =>
	...
	const renderCollection = (spec, providersBackstage, initialData) => {
		const pLabel = spec.label.map((label) =>
			renderLabel$3(label, providersBackstage),
		);
	...
	const renderColorInput = (
		...
		const pLabel = spec.label.map((label) =>
			renderLabel$3(label, sharedBackstage.providers),
		);
			...
			renderPanelButton(
				{
					...
					components: [],
					fetch: getFetch$1(
						colorInputBackstage.getColors(spec.storageKey),
						spec.storageKey,
	...
	const isElementScrollAtBottom = ({ scrollTop, scrollHeight, clientHeight }) =>
		...
		return Optional.from(
			!/^<!DOCTYPE (html|HTML)/.test(html) &&
				((!isChromium && !isSafari) ||
					(isNonNullable(body) &&
						(body.scrollTop !== 0 ||
							Math.abs(body.scrollHeight - body.clientHeight) > 1)))
				? body
				: doc.documentElement,
		);
	};
	const writeValue = (iframeElement, html, fallbackFn) => {
		...
		Optional.from(iframe.contentDocument).fold(fallbackFn, (doc) => {
			let lastScrollTop = 0;
			const isScrollAtBottom = getScrollingElement(doc, html)
				.map((el) => {
	...
	const writeValueThrottler = throttleInterval.map((interval) =>
		adaptable(writeValue, interval),
	);
		...
		return {
			...
			setValue: (frameComponent, html) => {
				if (cachedValue.get() !== html) {
					...
					if (stream) {
						writeValueThrottler.fold(
							constant$1(writeValue),
	...
	const renderIFrame = (spec, providersBackstage, initialData) => {
		...
		const factory = (newSpec) =>
			craft(Optional.from(containerBorderedClass), {
				uid: newSpec.uid,
				dom: {
					tag: "iframe",
					attributes,
	...
	const forceInitialSize = (comp) =>
		set$8(comp.element, "width", get$e(comp.element, "width"));

	const renderIcon$1 = (iconName, iconsProvider, behaviours) =>
		render$3(
			iconName,
			{
				tag: "span",
	...
	const renderIconFromPack$1 = (iconName, iconsProvider) =>
		renderIcon$1(iconName, iconsProvider, []);
	const renderReplaceableIconFromPack = (iconName, iconsProvider) =>
		renderIcon$1(iconName, iconsProvider, [Replacing.config({})]);
	...
	const renderCommonDropdown = (spec, prefix, sharedBackstage) => {
		...
		const optMemDisplayText = spec.text.map((text) =>
			record(renderLabel$1(text, prefix, sharedBackstage.providers)),
		);
							...
							optMemDisplayText
								...
								.each((displayText) => {
									Replacing.set(displayText, [
										text$2(sharedBackstage.providers.translate(se.event.text)),
									]);
								});
						}),
					...
					store: {
						...
						setValue: (select, newValue) => {
							...
							if (found.isSome()) {
								...
								select.element.dom.selectedIndex === -1 &&
								newValue === ""
							) {
	...
	const events$6 = (slideConfig, slideState) =>
		derive$2([
	...
	const renderLeafLabel = ({
		...
		const internalMenuButton = leaf.menu.map((btn) =>
			renderMenuButton(btn, "tox-mbtn", backstage, Optional.none(), visible),
		);
		...
		return Button.sketch({
			dom: {
				...
				classes: ["tox-tree--leaf__label", "tox-trbtn"].concat(
					visible ? ["tox-tree--leaf__label--visible"] : [],
				),
			},
			components,
			role: "treeitem",
			action: (button) => {
				...
				button
					.getSystem()
					.broadcastOn([`update-active-item-${treeId}`], { value: leaf.id });
			},
			eventOrder: {
				[keydown()]: [leafLabelEventsId, "keying"],
			},
			buttonBehaviours: derive$1([
				...
				config(leafLabelEventsId, [
					...
					run$1(keydown(), (comp, se) => {
						...
						if (isLeftArrowKey) {
							ancestor(comp.element, ".tox-tree--directory").each(
								(dirElement) => {
									comp
										.getSystem()
										.getByDom(dirElement)
										.each((dirComp) => {
											child(dirElement, ".tox-tree--directory__label").each(
												(dirLabelElement) => {
													dirComp
														.getSystem()
														.getByDom(dirLabelElement)
														.each(Focusing.focus);
												},
											);
										});
	...
	const renderDirectoryLabel = ({
		...
		const internalMenuButton = directory.menu.map((btn) =>
			renderMenuButton(btn, "tox-mbtn", backstage, Optional.none()),
		);
		const components = [
			{
				dom: {
		...
		return Button.sketch({
			dom: {
				...
				classes: ["tox-tree--directory__label", "tox-trbtn"].concat(
					visible ? ["tox-tree--directory__label--visible"] : [],
				),
			},
			components,
			action: toggleExpandChildren,
			eventOrder: {
				[keydown()]: [directoryLabelEventsId, "keying"],
			},
			buttonBehaviours: derive$1([
				...(visible ? [Tabstopping.config({})] : []),
							...
							ancestor(comp.element, ".tox-tree--directory").each(
								(directoryEle) => {
									comp
										...
										.each((directoryComp) => {
											if (
												(!Toggling.isOn(directoryComp) && isRightArrowKey) ||
												(Toggling.isOn(directoryComp) && isLeftArrowKey)
											) {
												toggleExpandChildren(comp);
												se.stop();
											} else if (
												isLeftArrowKey &&
												!Toggling.isOn(directoryComp)
											) {
												ancestor(
													directoryComp.element,
													".tox-tree--directory",
												).each((parentDirElement) => {
													child(
														parentDirElement,
														".tox-tree--directory__label",
													).each((parentDirLabelElement) => {
														directoryComp
															.getSystem()
															.getByDom(parentDirLabelElement)
															.each(Focusing.focus);
													});
												});
												se.stop();
	...
	const renderDirectory = ({
		...
		const computedChildrenComponents = (visible) =>
			children.map((item) => {
				return item.type === "leaf"
					? renderLeafLabel({
							leaf: item,
	...
	const renderTree = (spec, backstage) => {
		...
		const children = (selectedId, expandedIds) =>
			spec.items.map((item) => {
				return item.type === "leaf"
					? renderLeafLabel({
							leaf: item,
					...
					channels: {
						[`update-active-item-${treeId}`]: {
							onReceive: (comp, message) => {
								...
								Replacing.set(
									comp,
									children(Optional.some(message.value), expandedIds.get()),
								);
							},
						},
					},
						...
						return makeSandbox$1(detail, hotspot, {
							...
							onClose: () => {
								detail.lazyTypeaheadComp
									.get()
									.each((input) =>
										remove$7(input.element, "aria-activedescendant"),
									);
					...
					onHighlightItem: (_tmenu, menu, item) => {
						if (!detail.previewing.get()) {
							detail.lazyTypeaheadComp.get().each((input) => {
								...
								getOpt(item.element, "id").each((id) =>
									set$9(input.element, "aria-activedescendant", id),
								);
							});
						} else {
							detail.lazyTypeaheadComp.get().each((input) => {
	...
	const renderIconButtonSpec = (
		...
		const icon = spec.icon.map((iconName) =>
			renderIconFromPack$1(iconName, providersBackstage.icons),
		);
	...
	const isNormalFooterButtonSpec = (spec, buttonType) =>
		...
		buttonType === "cancel" ||
		buttonType === "submit";
	const isToggleButtonSpec = (spec, buttonType) =>
	...
	const renderToggleButton = (spec, providers) => {
		...
		const buttonType = spec.buttonType.getOr(
			!spec.primary ? "secondary" : "primary",
		);
		const buttonSpec = {
			...spec,
		...
		const tooltipAttributes = buttonSpec.tooltip
			.map((tooltip) => ({
		...
		const buttonTypeClasses = calculateClassesFromButtonType(
			buttonType !== null && buttonType !== void 0 ? buttonType : "secondary",
		);
		const showIconAndText = spec.icon.isSome() && spec.text.isSome();
		const dom = {
			tag: "button",
			classes: [
				...buttonTypeClasses.concat(
		...
		const iconComp = componentRenderPipeline([
		...
		const iconButtonSpec = renderCommonSpec(
			buttonSpec,
			Optional.some(action),
			extraBehaviours,
			dom,
	...
	const getItems = (fileType, input, urlBackstage) => {
		...
		const term =
			(_b =
				(_a =
					urlInputValue === null || urlInputValue === void 0
						? void 0
						: urlInputValue.meta) === null || _a === void 0
					? void 0
			...
			behaviours: derive$1([
				...
				Disabling.config({
					...
					onDisabled: (component) => {
						parentElement(component.element).each((element) =>
							add$2(element, "tox-checkbox--disabled"),
						);
					},
					onEnabled: (component) => {
						parentElement(component.element).each((element) =>
							remove$2(element, "tox-checkbox--disabled"),
						);
					},
				}),
				Tabstopping.config({}),
	...
	const getAnchors = (
		...
		return {
			...
			inlineBottomDialog: getInlineBottomDialogAnchor(
				editor.inline,
				contentAreaElement,
				lazyBottomAnchorBar,
				lazyUseEditableAreaAnchor,
		...
		const onElementScroll = capture(dos, "scroll", (evt) => {
			requestAnimationFrame(() => {
				...
				if (c !== undefined && c !== null) {
					const optScrollingContext = detectWhenSplitUiMode(
						editor,
						mothership.element,
					);
					const scrollers = optScrollingContext
						.map((sc) => [sc.element, ...sc.others])
						.getOr([]);
			...
			switch (mode) {
				case "bottom":
					return !isBottomCompletelyVisible(box, viewport.bounds)
						? Optional.some(forceBottomPosition(winBox, leftX, viewport))
						: Optional.none();
				case "top":
					return !isTopCompletelyVisible(box, viewport.bounds)
						? Optional.some(forceTopPosition(winBox, leftX, viewport))
						: Optional.none();
				default:
					return Optional.none();
			}
	...
	const getXYForRestoring = (pos, viewport) => {
		const priorY = viewport.optScrollEnv.fold(
			constant$1(pos.bounds.y),
			(scrollEnv) =>
				scrollEnv.scrollElmTop + (pos.bounds.y - scrollEnv.currentScrollTop),
		);
	...
	const getXYForSaving = (box, viewport) => {
		const priorY = viewport.optScrollEnv.fold(
			constant$1(box.y),
			(scrollEnv) =>
				box.y + scrollEnv.currentScrollTop - scrollEnv.scrollElmTop,
		);
		return SugarPosition(box.x, priorY);
	};
	const getPrior = (elem, viewport, state) =>
		state.getInitialPos().map((pos) => {
		...
		state.getInitialPos().bind((position) => {
			...
			switch (position.position) {
				...
				case "absolute":
					...
					const scrollDelta =
						(_a = offsetParent.dom.scrollTop) !== null && _a !== void 0
							? _a
							: 0;
					return Optional.some({
						morph: "absolute",
						positionCss: NuPositionCss(
							...
							get$g(position.style, "top").map(
								(_top) => box.y - offsetBox.y + scrollDelta,
							),
	...
	const tryMorphToOriginal = (elem, viewport, state) =>
	...
	const calculateMorphToOriginal = (component, viewport, state) => {
		const elem = component.element;
		return getPrior(elem, viewport, state).bind(({ box }) =>
	...
	const resetInternal = (component, config, state) => {
		...
		state.setDocked(false);
		const viewport = config.lazyViewport(component);
		calculateMorphToOriginal(component, viewport, state).each(
				...
				switch (staticOrAbsoluteMorph.morph) {
					...
					case "absolute": {
						morphToCoord(
							...
							config,
							state,
							staticOrAbsoluteMorph.positionCss,
						);
						break;
					}
				}
	...
	const events$4 = (dockInfo, dockState) =>
		derive$2([
			runOnSource(transitionend(), (component, simulatedEvent) => {
					...
					lazyContext: (comp) => {
						...
						return Optional.from(container).map((c) => {
							...
							return optScrollingContext.fold(
								...
								(scrollEnv) => {
									const constrainedBounds = constrain(
										box,
										getBoundsFrom(scrollEnv),
									);
				...
				lazyViewport: (comp) => {
					...
					return optScrollingContext.fold(
						() => {
							...
							const top =
								boundsWithoutOffset.y +
								(isDockedMode(comp, "top") ? offset : 0);
							const height =
								boundsWithoutOffset.height -
								(isDockedMode(comp, "bottom") ? offset : 0);
							return {
								bounds: bounds(
									boundsWithoutOffset.x,
									top,
									boundsWithoutOffset.width,
									height,
								),
								optScrollEnv: Optional.none(),
							};
	...
	const isBlocked = (component, blockingConfig, blockingState) =>
	...
	const factory$a = (detail, components, spec, externals) => {
		const memFloatingToolbarButton = record(
			FloatingToolbarButton.sketch({
				...
				onToggled: (comp, state) =>
					detail[state ? "onOpened" : "onClosed"](comp),
			}),
		);
		return {
			uid: detail.uid,
			...
			apis: {
				...
				toggleWithoutFocusing: (toolbar) => {
					memFloatingToolbarButton
						.getOpt(toolbar)
						.each(FloatingToolbarButton.toggleWithoutFocusing);
				},
				isOpen: (toolbar) =>
					memFloatingToolbarButton
						.getOpt(toolbar)
	...
	const renderButton = (spec, providers) => {
		...
		const buttonSpec = {
			...spec,
			name: isToggleButton
				? spec.text.getOr(spec.icon.getOr(""))
				: (_a = spec.text) !== null && _a !== void 0
					? _a
					: spec.icon.getOr(""),
		...
		const buttonTypeClasses = calculateClassesFromButtonType(
			(_b = spec.buttonType) !== null && _b !== void 0 ? _b : "secondary",
		);
		const optTranslatedText = isToggleButton
			? spec.text.map(providers.translate)
			: Optional.some(providers.translate(spec.text));
		const optTranslatedTextComponed = optTranslatedText.map(text$2);
		const tooltipAttributes = buttonSpec.tooltip
			.or(optTranslatedText)
			.map((tooltip) => ({
				"aria-label": providers.translate(tooltip),
				title: providers.translate(tooltip),
			}))
		...
		const hasIconAndText =
			spec.icon.isSome() && optTranslatedTextComponed.isSome();
		const dom = {
			tag: "button",
			classes: buttonTypeClasses
				.concat(
					...(spec.icon.isSome() && !hasIconAndText
						? ["tox-button--icon"]
						: []),
				)
				...
				.concat(
					...(spec.type === "togglebutton" && spec.active
						? ["tox-button--enabled"]
						: []),
				),
			attributes: tooltipAttributes,
	...
	const renderButtonsGroup = (spec, providers) => {
		return {
			...
			components: map$2(spec.buttons, (button) =>
				renderViewButton(button, providers),
			),
		};
	};
	...
	const getCurrentName = (slotContainer) => {
		return find$5(SlotContainer.getSlotNames(slotContainer), (name) =>
			SlotContainer.isShowing(slotContainer, name),
		);
	};
	...
	const runOnShow = (slotContainer, name) =>
		runOnPaneWithInstanceApi(slotContainer, name, View.getOnShow);
	const runOnHide = (slotContainer, name) =>
		runOnPaneWithInstanceApi(slotContainer, name, View.getOnHide);
	const factory$7 = (detail, spec) => {
		const setViews = (comp, viewConfigs) => {
			Replacing.set(comp, [
				makeSlotContainer(viewConfigs, spec.backstage.shared.providers),
			]);
		};
		...
		const toggleView = (comp, showMainView, hideMainView, name) => {
			return Composing.getCurrent(comp).exists((slotContainer) => {
				...
				const isTogglingCurrentView = optCurrentSlotName.exists(
				...
				if (exists) {
					...
					optCurrentSlotName.each((prevName) =>
						runOnHide(slotContainer, prevName),
					);
				}
				return exists;
			});
	...
	var ViewWrapper = single({
		...
		apis: {
			...
			toggleView: (apis, comp, outerContainer, editorCont, name) =>
				apis.toggleView(comp, outerContainer, editorCont, name),
	...
	const factory$6 = (detail, components, _spec) => {
		...
		const apis = {
			...
			toggleToolbarDrawerWithoutFocusing: (comp) => {
				parts$a.getPart(comp, detail, "toolbar").each((toolbar) => {
					mapFrom(
						toolbar.getApis().toggleWithoutFocusing,
						(toggleWithoutFocusing) => toggleWithoutFocusing(toolbar),
					);
				});
			},
			...
			whichView: (comp) => {
				return parts$a
					.getPart(comp, detail, "viewWrapper")
					.bind(ViewWrapper.whichView)
					.getOrNull();
			},
			hideMainView: (comp) => {
				...
				parts$a
					.getPart(comp, detail, "editorContainer")
					.each((editorContainer) => {
	...
	const createBespokeNumberInput = (editor, backstage, spec) => {
		...
		const getValueFromCurrentComp = (comp) =>
		...
		const changeValue = (f, fromInput, focusBack) => {
			...
			const oldStart = currentComp.map(
				(comp) => comp.element.dom.selectionStart - lenghtDelta,
			);
			const oldEnd = currentComp.map(
				(comp) => comp.element.dom.selectionEnd - lenghtDelta,
			);
			...
			currentComp.each((comp) => {
				...
				if (fromInput) {
					oldStart.each(
						(oldStart) => (comp.element.dom.selectionStart = oldStart),
					);
					oldEnd.each((oldEnd) => (comp.element.dom.selectionEnd = oldEnd));
				}
			});
		};
		const decrease = (fromInput, focusBack) =>
			changeValue((n, s) => n - s, fromInput, focusBack);
		const increase = (fromInput, focusBack) =>
			changeValue((n, s) => n + s, fromInput, focusBack);
		const goToParent = (comp) =>
			parentElement(comp.element).fold(Optional.none, (parent) => {
				focus$3(parent);
				return Optional.some(true);
			});
			...
			return Button.sketch({
				...
				buttonBehaviours: derive$1([
					...
					config(altExecuting, [
						...
						run$1(keydown(), (comp, se) => {
							if (
								se.event.raw.keyCode === Keys.space() ||
								se.event.raw.keyCode === Keys.enter()
							) {
								if (!Disabling.isDisabled(comp)) {
									action(false);
								}
		...
		const memMinus = record(
			makeStepperButton(
				(focusBack) => decrease(false, focusBack),
				"minus",
				"Decrease font size",
				["highlight-on-focus"],
			),
		);
		const memPlus = record(
			makeStepperButton(
				(focusBack) => increase(false, focusBack),
				"plus",
				"Increase font size",
				["highlight-on-focus"],
			),
				...
				config("input-wrapper-events", [
					run$1(mouseover(), (comp) => {
						each$1([memMinus, memPlus], (button) => {
							const buttonNode = SugarElement.fromDom(
								button.get(comp).element.dom,
							);
	...
	const getNumberInputSpec = (editor) => {
		...
		const updateInputValue = (comp) =>
			emitWith(comp, updateMenuText, { text: getCurrentValue() });
		return {
			updateInputValue,
			onAction: (format, focusBack) =>
			...
			getNewValue: (text, updateFunction) => {
				...
				const parsedText = parse(text, ["unsupportedLength", "empty"]).or(
					parse(getCurrentValue(), ["unsupportedLength", "empty"]),
				);
				const value = parsedText.map((res) => res.value).getOr(defaultValue);
				const defaultUnit = getFontSizeInputDefaultUnit(editor);
				const unit = parsedText
					.map((res) => res.unit)
					.filter((u) => u !== "")
					.getOr(defaultUnit);
	...
	const createFontSizeInputButton = (editor, backstage) =>
	...
	const renderCommonStructure = (
		...
		const optMemDisplayText = optText.map((text) =>
			record(renderLabel$1(text, "tox-tbtn", providersBackstage)),
		);
		const optMemDisplayIcon = optIcon.map((icon) =>
			record(renderReplaceableIconFromPack(icon, providersBackstage.icons)),
		);
		return {
			dom: {
				tag: "button",
				classes: ["tox-tbtn"].concat(
					optText.isSome() ? ["tox-tbtn--select"] : [],
				),
					...
					config(commonButtonDisplayEvent, [
						...
						run$1(updateMenuText, (comp, se) => {
							optMemDisplayText
								...
								.each((displayText) => {
									Replacing.set(displayText, [
										text$2(providersBackstage.translate(se.event.text)),
									]);
								});
						}),
						run$1(updateMenuIcon, (comp, se) => {
	...
	const renderCommonToolbarButton = (
		...
		return Button.sketch({
			...
			buttonBehaviours: {
				...
				[commonButtonDisplayEvent]:
					(_d = structure.buttonBehaviours) === null || _d === void 0
						? void 0
						: _d[commonButtonDisplayEvent],
			},
		});
	};
			...
			setText: (text) =>
				descendant(comp.element, "span").each((button) =>
					comp
						...
						.getByDom(button)
						.each((buttonComp) =>
							emitWith(buttonComp, updateMenuText, { text }),
						),
				),
			setIcon: (icon) =>
				descendant(comp.element, "span").each((button) =>
					comp
						...
						.getByDom(button)
						.each((buttonComp) =>
							emitWith(buttonComp, updateMenuIcon, { icon }),
						),
				),
		});
		const editorOffCell = Cell(noop);
	...
	const types = {
		...
		menubutton: renderFromBridge(createMenuButton, (s, backstage) =>
			renderMenuButton(s, "tox-tbtn", backstage, Optional.none(), false),
		),
	...
	const attachUiMotherships = (editor, uiRoot, uiRefs) => {
		if (isSplitUiMode(editor)) {
			attachSystemAfter(
				uiRefs.mainUi.mothership.element,
				uiRefs.popupUi.mothership,
			);
		}
	...
	const render$1 = async (editor, uiRefs, rawUiConfig, backstage, args) => {
		...
		editor.addQueryValueHandler("ToggleView", () => {
			var _a;
			return (_a = OuterContainer.whichView(outerContainer)) !== null &&
				_a !== void 0
				? _a
				: "";
		});
	...
	const InlineHeader = (
		...
		const updateChromePosition = (optToolbarWidth) => {
			floatContainer.on((container) => {
				...
				const offset = calcToolbarOffset(toolbar);
				const targetBounds = box$1(targetElm);
				const { top, left } = getOffsetParent$1(
					editor,
					mainUi.outerContainer.element,
				).fold(
					() => {
						return {
							top: isPositionedAtTop()
								? Math.max(
										targetBounds.y - get$d(container.element) + offset,
										0,
									)
								: targetBounds.bottom,
							left: targetBounds.x,
		...
		const getOffsetParent$1 = (editor, element) =>
			isSplitUiMode(editor) ? getOffsetParent(element) : Optional.none();
		...
		const restoreAndGetCompleteOuterContainerWidth = () => {
			if (!useFixedToolbarContainer) {
				const toolbarCurrentRightsidePosition =
					absolute$3(mainUi.outerContainer.element).left +
					getOuter$1(mainUi.outerContainer.element);
				if (
					toolbarCurrentRightsidePosition >=
						window.innerWidth - maximumDistanceToEdge ||
					getRaw(mainUi.outerContainer.element, "width").isSome()
				) {
		...
		const update = (stickyAction) => {
			...
			const optToolbarWidth = useFixedToolbarContainer
				? Optional.none()
		...
		const show = () => {
			...
			if (isSplitUiMode(editor)) {
				update((elem) =>
					Docking.isDocked(elem) ? Docking.reset(elem) : Docking.refresh(elem),
				);
			} else {
	...
	const render = async (editor, uiRefs, rawUiConfig, backstage, args) => {
		...
		const render = () => {
			...
			floatContainer.set(
				OuterContainer.getHeader(mainUi.outerContainer).getOrDie(),
			);
	...
	const LazyUiReferences = () => {
		...
		const getUiMotherships = () => {
			...
			return optDialogMothership.fold(
				...
				(dm) =>
					optPopupMothership.fold(
						() => [dm],
						(pm) => (eq(dm.element, pm.element) ? [dm] : [dm, pm]),
					),
			);
		};
		return {
	...
	const isVerticalOverlap = (a, b, threshold) =>
		...
		const shouldContextToolbarHide = () => {
			if (!canLaunchToolbar()) {
				...
				return (
					...
					!isVerticalOverlap(anchorBounds, contextToolbarBounds, 0.01)
				);
			}
		};
		const close = () => {
	...
	const onSetupOutdentState = (editor) =>
		onSetupEvent(editor, "NodeChange", (api) => {
			api.setEnabled(
				editor.queryCommandState("outdent") && editor.selection.isEditable(),
			);
		});
	...
	const events$3 = (dragConfig, dragState, updateStartState) => [
	...
	const renderWordCount = (editor, providersBackstage) => {
		const replaceCountText = (comp, count, mode) =>
			Replacing.set(comp, [
				text$2(providersBackstage.translate(["{0} " + mode, count[mode]])),
			]);
		return Button.sketch({
			dom: {
				tag: "button",
	...
	const renderStatusbar = (editor, providersBackstage) => {
		...
		const getTextComponents = () => {
			...
			const shouldRenderRightContainer =
				useBranding(editor) || editor.hasPlugin("wordcount");
			const getTextComponentClasses = () => {
				...
				return [
					shouldRenderRightContainer && !shouldRenderElementPath
						? flexEnd
						: flexStart,
				];
			};
	...
	const getLazyMothership = (label, singleton) =>
	...
	const setup$3 = (editor, setupForTheme) => {
			...
			return OuterContainer.parts.header({
				dom: {
					...
					classes: ["tox-editor-header"].concat(
						hasAnyContents ? [] : ["tox-editor-header--empty"],
					),
					...verticalDirAttributes,
				},
				components: flatten([
		...
		const setupShortcutsAndCommands = (outerContainer) => {
			...
			editor.addCommand("ToggleToolbarDrawer", (_ui, options) => {
				if (
					options === null || options === void 0 ? void 0 : options.skipFocus
				) {
		...
		const renderUIWithRefs = (uiRefs) => {
			...
			const { buttons, menuItems, contextToolbars, sidebars, views } =
		...
		const renderUI = () => {
			...
			const popupUi = isSplitUiMode(editor)
				? renderPopupUi()
				: reuseDialogUiForPopuUi(dialogUi);
			...
			components: [
				{
					...
					components: [
						{
							dom: fromHtml(
								`<p>${sanitizeHtmlString(providersBackstage.translate(message))}</p>`,
							),
						},
					],
				},
	...
	const renderTitle = (spec, dialogId, titleId, providersBackstage) => {
		const renderComponents = (data) => [
			text$2(providersBackstage.translate(data.title)),
		];
		return {
			dom: {
				tag: "div",
	...
	const getEventExtras = (lazyDialog, providers, extra) => ({
		...
		onBlock: (blockEvent) => {
			const headerHeight = descendant(
				lazyDialog().element,
				".tox-dialog__header",
			).map((header) => get$d(header));
			ModalDialog.setBusy(lazyDialog(), (_comp, bs) =>
				getBusySpec(blockEvent.message, bs, providers, headerHeight),
			);
		},
		onUnblock: () => {
	...
	const initDialog = (getInstanceApi, extras, getSink) => {
		...
		return [
			...
			fireApiEvent(formActionEvent, (api, spec, event, component) => {
				const focusIn = () =>
					component.getSystem().isConnected()
	...
	const getDialogApi = (access, doRedial, menuItemStates) => {
		...
		const redial = (d) => {
			withRoot((root) => {
				...
				const storedMenuButtons = mapMenuButtons(
			...
			() => instanceApi,
			{
				onBlock: (event) => {
					Blocking.block(dialog, (_comp, bs) => {
						const headerHeight = memHeader
							.getOpt(dialog)
							.map((dialog) => get$d(dialog.element));
						return getBusySpec(
							event.message,
							bs,
							backstage.shared.providers,
	...
	const renderUrlDialog = (internalDialog, extra, editor, backstage) => {
		...
		const dialogEvents = initUrlDialog(
			() => instanceApi,
			getEventExtras(() => dialog, backstage.shared.providers, extra),
		);
		const styles = {
	...
	const setup = (extras) => {
			...
			if (!isUndefined(params)) {
				if (params.inline === "toolbar") {
					return openInlineDialog(
						config,
						extras.backstages.popup.shared.anchors.inlineDialog(),
						closeWindow,
						params,
					);
				} else if (params.inline === "bottom") {
					return openBottomInlineDialog(
						config,
						extras.backstages.popup.shared.anchors.inlineBottomDialog(),
						closeWindow,
						params,
					);
					...
					return openInlineDialog(
						config,
						extras.backstages.popup.shared.anchors.cursor(),
						closeWindow,
						params,
					);
				}
		...
		const openInlineDialog = (config$1, anchor, closeWindow, windowParams) => {
			const factory = (contents, internalInitialData, dataValidator) => {
				...
				const isToolbarLocationTop =
					extras.backstages.popup.shared.header.isPositionedAtTop();
				const dialogInit = {
					dataValidator,
					initialData,
					internalDialog: contents,
		...
		const openBottomInlineDialog = (
			...
			const factory = (contents, internalInitialData, dataValidator) => {
				...
				inlineDialog.set(inlineDialogComp);
				const getInlineDialogBounds = () => {
					return extras.backstages.popup.shared
						.getSink()
						.toOptional()
						.bind((s) => {
							const optScrollingContext = detectWhenSplitUiMode(
							...
							const margin = 15;
							const bounds$1 = optScrollingContext
								.map((sc) => getBoundsFrom(sc))
								.getOr(win());
							const contentAreaContainer = box$1(
								SugarElement.fromDom(editor.getContentAreaContainer()),
							);
							const constrainedBounds = constrain(
							...
							return Optional.some(
								bounds(
									...
									constrainedBounds.y,
									constrainedBounds.width,
									constrainedBounds.height - margin,
								),
							);
						});
				};
	...
	var Theme = () => {
		global$a.add("silver", (editor) => {
			...
			const getNotificationManagerImpl = () =>
