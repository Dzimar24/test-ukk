...
const queries = (state) => ({
	...
	GET_ACTIVE_ITEM: (query) =>
		getItemByQuery(getActiveItems(state.items), query),
...
const createFileProcessorFunction =
	(apiUrl, action, name, options) =>
	(file, metadata, load, error, progress, abort, transfer) => {
		// no file received
		if (!file) return;
...
const dragover = (root, clients) => (e) => {
	...
	requestDataTransferItems(dataTransfer).then((items) => {
		...
		clients.some((client) => {
			const { filterElement, element, onenter, onexit, ondrag, allowdrop } =
				client;
